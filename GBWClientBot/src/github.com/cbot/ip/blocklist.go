...
func NewWblist(whiteListFName string,blackListFName string,whiteListEntries []string,
		   blakListEntries []string,ignoreInvalidHosts bool) error {
	...
	if whiteListFName!="" || len(whiteListEntries) > 0 {
		...
		con = NewConstraint(ADDR_DISALLOWED)
		...
		con = NewConstraint(ADDR_ALLOWED)
	}
	...
	if allowed == 0 {
		...
		return fmt.Errorf("WBList no addresses are eligible to be scanned in the " +
